...
//				return
			if(H.glasses)
				...
				return
			if(H.head && (H.head.body_parts_covered & HEADCOVERSMOUTH))
			...
			if(!H || !src || !H.organs_by_name["head"] || H.glasses || (H.head && (H.head.body_parts_covered & HEADCOVERSMOUTH)))
				return
			...
			// Repeat failure checks.
			if(!H || !src || !H.organs_by_name["head"] || H.wear_mask || (H.head && (H.head.body_parts_covered & HEADCOVERSMOUTH)))
				return
			...
			if(!H.handcuffed)
				H.handcuffed = new /obj/item/weapon/handcuffs/cable/tape(H)
				H.update_inv_handcuffed()	//update the handcuffs overlay
		return
