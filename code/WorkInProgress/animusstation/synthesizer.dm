			...
			for(var/beat in text2list(lowertext(line), ","))
				...
				for(var/note in text2list(notes[1], "-"))
					...
					if (classic)
						...
						for (var/mob/hearer in heard_by)
							note_sound.x = (src.x - hearer.x)*ThreeDMod
							note_sound.z = (src.y - hearer.y)*ThreeDMod
							hearer << note_sound
...
/obj/structure/device/piano/synthesizer/Topic(href, href_list)
	...
	if(href_list["newsong"])
		song = new()
	if(href_list["TDChange"])
		ThreeDMod = href_list["new_value"]
	else if(song)
