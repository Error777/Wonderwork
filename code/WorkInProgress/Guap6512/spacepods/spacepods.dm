...
/obj/spacepod/proc/add_cabin()
	...
	cabin_air.adjust((6*ONE_ATMOSPHERE)*200/(R_IDEAL_GAS_EQUATION*cabin_air.temperature))
	return cabin_air
...
/obj/spacepod/proc/Process_Spacemove(var/movement_dir = 0, var/check_drift = 0, mob/user)
...
/obj/spacepod/proc/newtonian_move(direction) //Only moves the object if it's under no gravity
...
/turf/Enter(var/obj/spacepod/S)
	if(!istype(S))
		return ..()
	if(!istype(src, /turf/space) && !istype(src, /turf/simulated/floor/engine) && !istype(src,/turf/simulated/floor/plating))
		return 0
	else
		return ..()
