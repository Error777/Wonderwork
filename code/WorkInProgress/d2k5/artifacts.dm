...
/obj/structure/artifact
	...
	attackby(obj/item/weapon/W as obj, mob/living/user as mob)
		...
		if (src.trigger == "heat")
			if (istype(W,/obj/item/device/assembly/igniter)) src.Artifact_Activate()
...
/obj/structure/artifact/proc/Artifact_Activate()
	src.activated = 1
	for(var/mob/O in viewers(src, null))
		O.show_message(text("<b>[]</b> activates!", src), 1)
	var/artioverlay = src.icon_state + "-[rand(1,3)]"
	src.overlays += image('icons/artifacts.dmi', artioverlay)

/obj/structure/artifact/proc/Artifact_Contact(var/mob/living/carbon/human/user as mob)
	// Trigger Code
	...
	if (src.effectmode == "contact" && src.activated && src.charged)
		switch(src.effecttype)
			...
			if("forcefield")
				new /obj/effect/forcefield(locate(src.x + 2,src.y,src.z))
				new /obj/effect/forcefield(locate(src.x + 2,src.y + 1,src.z))
				new /obj/effect/forcefield(locate(src.x + 2,src.y + 2,src.z))
				new /obj/effect/forcefield(locate(src.x + 2,src.y - 1,src.z))
				new /obj/effect/forcefield(locate(src.x + 2,src.y - 2,src.z))
				new /obj/effect/forcefield(locate(src.x - 2,src.y,src.z))
				new /obj/effect/forcefield(locate(src.x - 2,src.y + 1,src.z))
				...
				new /obj/effect/forcefield(locate(src.x - 2,src.y - 1,src.z))
				new /obj/effect/forcefield(locate(src.x - 2,src.y - 2,src.z))
				new /obj/effect/forcefield(locate(src.x,src.y + 2,src.z))
				new /obj/effect/forcefield(locate(src.x + 1,src.y + 2,src.z))
				new /obj/effect/forcefield(locate(src.x - 1,src.y + 2,src.z))
				new /obj/effect/forcefield(locate(src.x,src.y - 2,src.z))
				new /obj/effect/forcefield(locate(src.x + 1,src.y - 2,src.z))
				new /obj/effect/forcefield(locate(src.x - 1,src.y - 2,src.z))
...
obj/machinery/artifact_analyser_old
	...
	Topic(href, href_list)
		if(href_list["ops"])
			...
			if(operation == 1) // Analyse Artifact
				var/findarti = 0
				for(var/obj/structure/artifact/A in range(1,src))
					findarti++
				if (findarti == 1)
					for(var/obj/structure/artifact/A in range(1,src))
						A.anchored = 1
...
/proc/artifact_checkgood(var/obj/structure/artifact/A)
	switch(A.effecttype)
		if("healing") return 1
		if("injure") return 0
		if("stun") return 0
