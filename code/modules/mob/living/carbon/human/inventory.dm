...
/obj/effect/equip_e/human/process()
	...
	if ((item && !( L.Find(place) )))
		...
		switch(place)
			...
			if("l_ear")
				target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has had their ear item ([target.l_ear]) removed by [source.name] ([source.ckey])</font>")
				source.attack_log += text("\[[time_stamp()]\] <font color='red'>Attempted to remove [target.name]'s ([target.ckey]) ear item ([target.l_ear])</font>")
				if(target.l_ear && !target.l_ear.canremove)
					message = "\red <B>[source] fails to take off \a [target.l_ear] from [target]'s ears!</B>"
					return
				else
					message = "\red <B>[source] is trying to take off the [target.l_ear] from [target]'s ears!</B>"
			if("r_ear")
				...
				if(target.l_ear && !target.r_ear.canremove)
					...
					message = "\red <B>[source] is trying to take off the [target.r_ear] from [target]'s ears!</B>"
			if("head")
